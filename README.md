...
- **Structured output**: Pydantic schemas for glossary, domain rules, search queries, and answers with citations

## Features

- **Ingestion**: PDF → Markdown (with OCR when needed) → header-based chunks → ChromaDB upsert (with `source`, `h1`/`h2`/`h3`, `chunk_index` metadata). Ollama extracts **glossary** and **domain rules** into `knowledge.md` (both sections are merged and compacted when the file grows).
...
- **Query**: Read `knowledge.md` (glossary + domain rules) → Ollama generates 3 vector-search queries → ChromaDB retrieval (top ~50) → **cross-encoder rerank** (top 5) → for each of the 5, add **1 sibling above + 1 below** (same section, by `chunk_index`) → dedupe, sort, cap at **15 chunks** → Ollama for final answer with citations.
...
- **Knowledge compaction**: When `knowledge.md` reaches ~50% of a 128K-token context (≈256K chars), Ollama compacts both sections: deduplicate and shorten glossary and domain rules. Runs after merge and before query so the file stays within context.
...
- **Knowledge Base** — View the current `knowledge.md` (glossary and domain rules).
- **Search Vector DB** — Run a custom vector search over stored chunks (no LLM); see source and text per hit.
- **Settings (sidebar)** — Set `chroma_path`, `knowledge_path`, `model`, `collection_name`, and **ingestion folder**; click Apply to re-create the RAG with new params. Uploaded PDFs are saved in the ingestion folder, and each PDF’s extracted markdown is saved there as `{filename}.md`.
...
rag.ingest_document("document.pdf", markdown_output_dir="./ingested_markdown")  # optional: save markdown to folder
...
- `knowledge.md` — created/updated by ingestion (glossary and domain rules); compacted automatically when it grows too large
